...
const CollectionPage = () => {
  const selectedCollection = useSelector(
    (state: RootState) => state.collection.selectedCollection
  );
  const favorites = currentUser.favorites?.filter(
    (f) => f.id == selectedCollection
  );

  return (
    <div className="space-y-6 p-4">
      {favorites?.map((fav) => {
        const threadsForFav = twirls.filter((thread) =>
          fav.twirlIds.includes(thread.id)
        );
        return (
          <div key={fav.id}>
            {/* Favorite Header */}
            ...
            <div className="space-y-4">
              {threadsForFav.map((thread) => (
                <TwirlBox key={thread.id} twirl={thread} isReply={false} />
              ))}
            </div>
          </div>
        );
